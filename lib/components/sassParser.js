  ...
  fileContentsArr.forEach((value) => {
    ...
    dividedHead.forEach((headvalue) => {
      if (head.length > 0) {
        ...
        let cleanHeadValue = headvalue.trim()
...
const convertCssToObject = (cssContent) => {
  ...
  try {
    ...
    dbg('Error', 'The source CSS is not valid', colors.RED);
  ...
  try {
    ...
    dbg('Error', 'There was a problem converting the CSS to an Object', colors.RED);
